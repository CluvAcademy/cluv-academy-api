 ...
 */
@RestController
@RequestMapping("private/ListaAsistencia")
public class ListaAsistenciaRest {

    @Autowired
    private ListaAsistenciaService service;
    ...
    public ResponseEntity<?> create(@RequestBody ListaAsistenciaDto obj) throws Exception {
        ListaAsistenciaDto dto = service.create(obj);
        ResponseGenerico<ListaAsistenciaDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<ListaAsistenciaDto> response = new ResponseGenerico<>();
        try {
            List<ListaAsistenciaDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<ListaAsistenciaDto> response = new ResponseGenerico<>();
        try {
            ListaAsistenciaDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
