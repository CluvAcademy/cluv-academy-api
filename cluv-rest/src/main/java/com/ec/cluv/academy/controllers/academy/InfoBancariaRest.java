 ...
 */
@RestController
@RequestMapping("private/InfoBancaria")
public class InfoBancariaRest {

    @Autowired
    private InfoBancariaService service;
    ...
    public ResponseEntity<?> create(@RequestBody InfoBancariaDto obj) throws Exception {
        InfoBancariaDto dto = service.create(obj);
        ResponseGenerico<InfoBancariaDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<InfoBancariaDto> response = new ResponseGenerico<>();
        try {
            List<InfoBancariaDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<InfoBancariaDto> response = new ResponseGenerico<>();
        try {
            InfoBancariaDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
