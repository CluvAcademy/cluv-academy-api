...
import com.ec.cluv.academy.service.service.academy.FacturaService;
...
public class FacturaRest {
    ...
    public ResponseEntity<?> create(@RequestBody FacturaDto obj) throws Exception {
        FacturaDto dto = service.create(obj);
        ResponseGenerico<FacturaDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<FacturaDto> response = new ResponseGenerico<>();
        try {
            List<FacturaDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<FacturaDto> response = new ResponseGenerico<>();
        try {
            FacturaDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
