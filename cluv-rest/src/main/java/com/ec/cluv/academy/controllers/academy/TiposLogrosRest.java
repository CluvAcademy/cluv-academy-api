 ...
 */
@RestController
@RequestMapping("private/TiposLogros")
public class TiposLogrosRest {

    @Autowired
    private TiposLogrosService service;
    ...
    public ResponseEntity<?> create(@RequestBody TiposLogrosDto obj) throws Exception {
        TiposLogrosDto dto = service.create(obj);
        ResponseGenerico<TiposLogrosDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<TiposLogrosDto> response = new ResponseGenerico<>();
        try {
            List<TiposLogrosDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<TiposLogrosDto> response = new ResponseGenerico<>();
        try {
            TiposLogrosDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
