 ...
 */
@RestController
@RequestMapping("private/ReconocimientoClub")
public class ReconocimientoClubRest {

    @Autowired
    private ReconocimientoClubService service;
    ...
    public ResponseEntity<?> create(@RequestBody ReconocimientoClubDto obj) throws Exception {
        ReconocimientoClubDto dto = service.create(obj);
        ResponseGenerico<ReconocimientoClubDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<ReconocimientoClubDto> response = new ResponseGenerico<>();
        try {
            List<ReconocimientoClubDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<ReconocimientoClubDto> response = new ResponseGenerico<>();
        try {
            ReconocimientoClubDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
