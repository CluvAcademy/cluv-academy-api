 ...
 */
@RestController
@RequestMapping("private/Resenias")
public class ReseniasRest {

    @Autowired
    private ReseniasService service;
    ...
    public ResponseEntity<?> create(@RequestBody ReseniasDto obj) throws Exception {
        ReseniasDto dto = service.create(obj);
        ResponseGenerico<ReseniasDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<ReseniasDto> response = new ResponseGenerico<>();
        try {
            List<ReseniasDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<ReseniasDto> response = new ResponseGenerico<>();
        try {
            ReseniasDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
