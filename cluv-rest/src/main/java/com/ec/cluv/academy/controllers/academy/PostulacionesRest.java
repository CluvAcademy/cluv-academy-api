 ...
 */
@RestController
@RequestMapping("private/Postulaciones")
public class PostulacionesRest {

    @Autowired
    private PostulacionesService service;
    ...
    public ResponseEntity<?> create(@RequestBody PostulacionesDto obj) throws Exception {
        PostulacionesDto dto = service.create(obj);
        ResponseGenerico<PostulacionesDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<PostulacionesDto> response = new ResponseGenerico<>();
        try {
            List<PostulacionesDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<PostulacionesDto> response = new ResponseGenerico<>();
        try {
            PostulacionesDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
