 ...
 */
@RestController
@RequestMapping("private/EstadoClase")
public class EstadoClaseRest {

    @Autowired
    private EstadoClaseService service;
    ...
    public ResponseEntity<?> create(@RequestBody EstadoClaseDto obj) throws Exception {
        EstadoClaseDto dto = service.create(obj);
        ResponseGenerico<EstadoClaseDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<EstadoClaseDto> response = new ResponseGenerico<>();
        try {
            List<EstadoClaseDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<EstadoClaseDto> response = new ResponseGenerico<>();
        try {
            EstadoClaseDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
