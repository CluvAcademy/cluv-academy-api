...
public class TipoClaseRest {

    @Autowired
    private TipoClaseService service;
    ...
    public ResponseEntity<?> create(@RequestBody TipoClaseDto obj) throws Exception {
        TipoClaseDto dto = service.create(obj);
        ResponseGenerico<TipoClaseDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<TipoClaseDto> response = new ResponseGenerico<>();
        try {
            List<TipoClaseDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<TipoClaseDto> response = new ResponseGenerico<>();
        try {
            TipoClaseDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
