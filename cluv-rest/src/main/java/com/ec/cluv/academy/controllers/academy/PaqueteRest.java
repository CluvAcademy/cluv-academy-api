 ...
 */
@RestController
@RequestMapping("private/Paquete")
public class PaqueteRest {
    ...
    public ResponseEntity<?> create(@RequestBody PaqueteDto obj) throws Exception {
        PaqueteDto dto = service.create(obj);
        ResponseGenerico<PaqueteDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<PaqueteDto> response = new ResponseGenerico<>();
        try {
            List<PaqueteDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<PaqueteDto> response = new ResponseGenerico<>();
        try {
            PaqueteDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
