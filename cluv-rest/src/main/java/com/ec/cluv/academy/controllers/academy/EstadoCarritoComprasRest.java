 ...
 */
@RestController
@RequestMapping("private/EstadoCarritoCompras")
public class EstadoCarritoComprasRest {

    @Autowired
    private EstadoCarritoComprasService service;
    ...
    public ResponseEntity<?> create(@RequestBody EstadoCarritoComprasDto obj) throws Exception {
        EstadoCarritoComprasDto dto = service.create(obj);
        ResponseGenerico<EstadoCarritoComprasDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<EstadoCarritoComprasDto> response = new ResponseGenerico<>();
        try {
            List<EstadoCarritoComprasDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<EstadoCarritoComprasDto> response = new ResponseGenerico<>();
        try {
            EstadoCarritoComprasDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
