 ...
 */
@RestController
@RequestMapping("private/ForoClub")
public class ForoClubRest {

    @Autowired
    private ForoClubService service;
    ...
    public ResponseEntity<?> create(@RequestBody ForoClubDto obj) throws Exception {
        ForoClubDto dto = service.create(obj);
        ResponseGenerico<ForoClubDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<ForoClubDto> response = new ResponseGenerico<>();
        try {
            List<ForoClubDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<ForoClubDto> response = new ResponseGenerico<>();
        try {
            ForoClubDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
