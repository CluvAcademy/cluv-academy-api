 ...
 */
@RestController
@RequestMapping("private/EstadoEstudiante")
public class EstadoEstudianteRest {

    @Autowired
    private EstadoEstudianteService service;
    ...
    public ResponseEntity<?> create(@RequestBody EstadoEstudianteDto obj) throws Exception {
        EstadoEstudianteDto dto = service.create(obj);
        ResponseGenerico<EstadoEstudianteDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<EstadoEstudianteDto> response = new ResponseGenerico<>();
        try {
            List<EstadoEstudianteDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<EstadoEstudianteDto> response = new ResponseGenerico<>();
        try {
            EstadoEstudianteDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
