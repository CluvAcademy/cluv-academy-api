 ...
 */
@RestController
@RequestMapping("private/Materia")
public class MateriaRest {
    ...
    public ResponseEntity<?> create(@RequestBody MateriaDto obj) throws Exception {
        MateriaDto dto = service.create(obj);
        ResponseGenerico<MateriaDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<MateriaDto> response = new ResponseGenerico<>();
        try {
            List<MateriaDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<MateriaDto> response = new ResponseGenerico<>();
        try {
            MateriaDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
