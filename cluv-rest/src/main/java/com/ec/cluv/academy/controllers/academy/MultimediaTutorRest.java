 ...
 */
@RestController
@RequestMapping("private/MultimediaTutor")
public class MultimediaTutorRest {

    @Autowired
    private MultimediaTutorService service;
    ...
    public ResponseEntity<?> create(@RequestBody MultimediaTutorDto obj) throws Exception {
        MultimediaTutorDto dto = service.create(obj);
        ResponseGenerico<MultimediaTutorDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<MultimediaTutorDto> response = new ResponseGenerico<>();
        try {
            List<MultimediaTutorDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<MultimediaTutorDto> response = new ResponseGenerico<>();
        try {
            MultimediaTutorDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
