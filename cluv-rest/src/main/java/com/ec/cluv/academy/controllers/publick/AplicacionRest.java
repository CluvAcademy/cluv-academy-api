...
public class AplicacionRest {
    ...
    /**
     * REST para guardar o actualizar AplicacionDto
     *
     * @return guardar
     */
    @PostMapping(value = "guardarAplicacion")
    public ResponseEntity<?> create(@RequestBody AplicacionDto obj) throws Exception {
        AplicacionDto dto = service.create(obj);
        ResponseGenerico<AplicacionDto> response = new ResponseGenerico<>();
        try {
     ...
     */
    @GetMapping(value = "listaAplicacion")
    public ResponseEntity<?> listar() {
        ResponseGenerico<AplicacionDto> response = new ResponseGenerico<>();
        try {
            List<AplicacionDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    @GetMapping(value = "buscarPorIdAplicacion/{id}")
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<AplicacionDto> response = new ResponseGenerico<>();
        try {
            AplicacionDto dto = service.findById(id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
