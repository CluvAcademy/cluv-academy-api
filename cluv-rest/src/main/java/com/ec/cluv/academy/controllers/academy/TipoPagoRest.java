 ...
 */
@RestController
@RequestMapping("private/TipoPago")
public class TipoPagoRest {
    ...
    public ResponseEntity<?> create(@RequestBody TipoPagoDto obj) throws Exception {
        TipoPagoDto dto = service.create(obj);
        ResponseGenerico<TipoPagoDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<TipoPagoDto> response = new ResponseGenerico<>();
        try {
            List<TipoPagoDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<TipoPagoDto> response = new ResponseGenerico<>();
        try {
            TipoPagoDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
