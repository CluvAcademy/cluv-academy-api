 ...
 */
@RestController
@RequestMapping("private/TipoPaquete")
public class TipoPaqueteRest {

    @Autowired
    private TipoPaqueteService service;
    ...
    public ResponseEntity<?> create(@RequestBody TipoPaqueteDto obj) throws Exception {
        TipoPaqueteDto dto = service.create(obj);
        ResponseGenerico<TipoPaqueteDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<TipoPaqueteDto> response = new ResponseGenerico<>();
        try {
            List<TipoPaqueteDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<TipoPaqueteDto> response = new ResponseGenerico<>();
        try {
            TipoPaqueteDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
