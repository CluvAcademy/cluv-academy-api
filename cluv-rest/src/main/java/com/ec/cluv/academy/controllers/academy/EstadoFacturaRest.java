 ...
 */
@RestController
@RequestMapping("private/EstadoFactura")
public class EstadoFacturaRest {

    @Autowired
    private EstadoFacturaService service;
    ...
    public ResponseEntity<?> create(@RequestBody EstadoFacturaDto obj) throws Exception {
        EstadoFacturaDto dto = service.create(obj);
        ResponseGenerico<EstadoFacturaDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<EstadoFacturaDto> response = new ResponseGenerico<>();
        try {
            List<EstadoFacturaDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<EstadoFacturaDto> response = new ResponseGenerico<>();
        try {
            EstadoFacturaDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
