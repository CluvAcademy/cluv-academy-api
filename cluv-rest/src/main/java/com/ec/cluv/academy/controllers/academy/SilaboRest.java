 ...
 */
@RestController
@RequestMapping("private/Silabo")
public class SilaboRest {

    @Autowired
    private SilaboService service;
    ...
    public ResponseEntity<?> create(@RequestBody SilaboDto obj) throws Exception {
        SilaboDto dto = service.create(obj);
        ResponseGenerico<SilaboDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<SilaboDto> response = new ResponseGenerico<>();
        try {
            List<SilaboDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<SilaboDto> response = new ResponseGenerico<>();
        try {
            SilaboDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
