 ...
 */
@RestController
@RequestMapping("private/DiasHorario")
public class DiasHorarioRest {

    @Autowired
    private DiasHorarioService service;
    ...
    public ResponseEntity<?> create(@RequestBody DiasHorarioDto obj) throws Exception {
        DiasHorarioDto dto = service.create(obj);
        ResponseGenerico<DiasHorarioDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<DiasHorarioDto> response = new ResponseGenerico<>();
        try {
            List<DiasHorarioDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<DiasHorarioDto> response = new ResponseGenerico<>();
        try {
            DiasHorarioDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
