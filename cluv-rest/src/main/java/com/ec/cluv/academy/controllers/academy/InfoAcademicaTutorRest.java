 ...
 */
@RestController
@RequestMapping("private/InfoAcademicaTutor")
public class InfoAcademicaTutorRest {

    @Autowired
    private InfoAcademicaTutorService service;
    ...
    public ResponseEntity<?> create(@RequestBody InfoAcademicaTutorDto obj) throws Exception {
        InfoAcademicaTutorDto dto = service.create(obj);
        ResponseGenerico<InfoAcademicaTutorDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<InfoAcademicaTutorDto> response = new ResponseGenerico<>();
        try {
            List<InfoAcademicaTutorDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<InfoAcademicaTutorDto> response = new ResponseGenerico<>();
        try {
            InfoAcademicaTutorDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
