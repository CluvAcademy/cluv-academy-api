 ...
 */
@RestController
@RequestMapping("private/EstadoActividadTutor")
public class EstadoActividadTutorRest {

    @Autowired
    private EstadoActividadTutorService service;
    ...
    public ResponseEntity<?> create(@RequestBody EstadoActividadTutorDto obj) throws Exception {
        EstadoActividadTutorDto dto = service.create(obj);
        ResponseGenerico<EstadoActividadTutorDto> response = new ResponseGenerico<>();
        try {
    ...
    public ResponseEntity<?> listar() {
        ResponseGenerico<EstadoActividadTutorDto> response = new ResponseGenerico<>();
        try {
            List<EstadoActividadTutorDto> lista = service.findAll();
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ResponseGenerico<EstadoActividadTutorDto> response = new ResponseGenerico<>();
        try {
            EstadoActividadTutorDto dto = service.findById((long) id);
            response.setCodigoRespuestaName(HttpStatus.OK.name());
            response.setCodigoRespuestaValue(HttpStatus.OK.value());
            response.setObjeto(dto);
            response.setMensaje(EnumMessages.REGISTRO_EXISTENTE.name());
